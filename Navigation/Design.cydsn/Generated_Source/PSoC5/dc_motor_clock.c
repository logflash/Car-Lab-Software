...
/*******************************************************************************
* Function Name: dc_motor_clock_Start
********************************************************************************
*
* Summary:
...
void dc_motor_clock_Start(void) 
{
    ...
    dc_motor_clock_CLKEN |= dc_motor_clock_CLKEN_MASK;
	dc_motor_clock_CLKSTBY |= dc_motor_clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: dc_motor_clock_Stop
********************************************************************************
*
* Summary:
...
void dc_motor_clock_Stop(void) 
{
    ...
    dc_motor_clock_CLKEN &= (uint8)(~dc_motor_clock_CLKEN_MASK);
	dc_motor_clock_CLKSTBY &= (uint8)(~dc_motor_clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: dc_motor_clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((dc_motor_clock_CLKEN & dc_motor_clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = dc_motor_clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = dc_motor_clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* dc_motor_clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(dc_motor_clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(dc_motor_clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: dc_motor_clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: dc_motor_clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = dc_motor_clock_GetSourceRegister();
    uint16 oldDivider = dc_motor_clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = dc_motor_clock_CLKEN & dc_motor_clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(dc_motor_clock_DIV_PTR, clkDivider);
                dc_motor_clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                dc_motor_clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(dc_motor_clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((dc_motor_clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: dc_motor_clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(dc_motor_clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: dc_motor_clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    dc_motor_clock_MOD_SRC |= modeBitMask & (uint8)dc_motor_clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: dc_motor_clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    dc_motor_clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(dc_motor_clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: dc_motor_clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return dc_motor_clock_MOD_SRC & (uint8)(dc_motor_clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: dc_motor_clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = dc_motor_clock_GetDividerRegister();
    uint8 oldSrc = dc_motor_clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        dc_motor_clock_MOD_SRC |= CYCLK_SSS;
        dc_motor_clock_MOD_SRC =
            (dc_motor_clock_MOD_SRC & (uint8)(~dc_motor_clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: dc_motor_clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return dc_motor_clock_MOD_SRC & dc_motor_clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: dc_motor_clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    dc_motor_clock_PHASE = clkPhase & dc_motor_clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: dc_motor_clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return dc_motor_clock_PHASE & dc_motor_clock_PHASE_MASK;
}
