...
/*******************************************************************************
* Function Name: throttle_clock_Start
********************************************************************************
*
* Summary:
...
void throttle_clock_Start(void) 
{
    ...
    throttle_clock_CLKEN |= throttle_clock_CLKEN_MASK;
	throttle_clock_CLKSTBY |= throttle_clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: throttle_clock_Stop
********************************************************************************
*
* Summary:
...
void throttle_clock_Stop(void) 
{
    ...
    throttle_clock_CLKEN &= (uint8)(~throttle_clock_CLKEN_MASK);
	throttle_clock_CLKSTBY &= (uint8)(~throttle_clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: throttle_clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((throttle_clock_CLKEN & throttle_clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = throttle_clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = throttle_clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* throttle_clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(throttle_clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(throttle_clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: throttle_clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: throttle_clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = throttle_clock_GetSourceRegister();
    uint16 oldDivider = throttle_clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = throttle_clock_CLKEN & throttle_clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(throttle_clock_DIV_PTR, clkDivider);
                throttle_clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                throttle_clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(throttle_clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((throttle_clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: throttle_clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(throttle_clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: throttle_clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    throttle_clock_MOD_SRC |= modeBitMask & (uint8)throttle_clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: throttle_clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    throttle_clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(throttle_clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: throttle_clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return throttle_clock_MOD_SRC & (uint8)(throttle_clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: throttle_clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = throttle_clock_GetDividerRegister();
    uint8 oldSrc = throttle_clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        throttle_clock_MOD_SRC |= CYCLK_SSS;
        throttle_clock_MOD_SRC =
            (throttle_clock_MOD_SRC & (uint8)(~throttle_clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: throttle_clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return throttle_clock_MOD_SRC & throttle_clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: throttle_clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    throttle_clock_PHASE = clkPhase & throttle_clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: throttle_clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return throttle_clock_PHASE & throttle_clock_PHASE_MASK;
}
