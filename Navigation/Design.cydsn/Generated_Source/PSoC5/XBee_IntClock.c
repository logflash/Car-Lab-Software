...
void XBee_IntClock_Start(void) 
{
    ...
    XBee_IntClock_CLKEN |= XBee_IntClock_CLKEN_MASK;
	XBee_IntClock_CLKSTBY |= XBee_IntClock_CLKSTBY_MASK;
}
...
void XBee_IntClock_Stop(void) 
{
    ...
    XBee_IntClock_CLKEN &= (uint8)(~XBee_IntClock_CLKEN_MASK);
	XBee_IntClock_CLKSTBY &= (uint8)(~XBee_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: XBee_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((XBee_IntClock_CLKEN & XBee_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = XBee_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = XBee_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* XBee_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(XBee_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(XBee_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: XBee_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: XBee_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = XBee_IntClock_GetSourceRegister();
    uint16 oldDivider = XBee_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = XBee_IntClock_CLKEN & XBee_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(XBee_IntClock_DIV_PTR, clkDivider);
                XBee_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                XBee_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(XBee_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((XBee_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    return CY_GET_REG16(XBee_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: XBee_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    XBee_IntClock_MOD_SRC |= modeBitMask & (uint8)XBee_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: XBee_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    XBee_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(XBee_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: XBee_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return XBee_IntClock_MOD_SRC & (uint8)(XBee_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: XBee_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = XBee_IntClock_GetDividerRegister();
    uint8 oldSrc = XBee_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        XBee_IntClock_MOD_SRC |= CYCLK_SSS;
        XBee_IntClock_MOD_SRC =
            (XBee_IntClock_MOD_SRC & (uint8)(~XBee_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: XBee_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return XBee_IntClock_MOD_SRC & XBee_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: XBee_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    XBee_IntClock_PHASE = clkPhase & XBee_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: XBee_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return XBee_IntClock_PHASE & XBee_IntClock_PHASE_MASK;
}
